...
inline uint32_t adreno_a5xx_perfcounter_get_group(
    hpc_gpu_adreno_a5xx_perfcounter_t perfcounter) {
  return perfcounter >> 8u;
}

inline uint32_t adreno_a5xx_perfcounter_get_selector(
    hpc_gpu_adreno_a5xx_perfcounter_t perfcounter) {
...
int hpc_gpu_adreno_a5xx_create_context(
  ...
  int status =
      adreno_create_context(num_counters, counters, allocator, out_context);
  ...
  switch (series) {
    ...
    case HPC_GPU_ADRENO_SERIES_A5XX:
      for (int i = 0; i < num_counters; ++i) {
        uint32_t group_id = adreno_a5xx_perfcounter_get_group(counters[i]);
        uint32_t countable_selector =
            adreno_a5xx_perfcounter_get_selector(counters[i]);
