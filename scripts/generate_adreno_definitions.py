...
# C enum.
ADRENO_MAX_COUNTER_COUNT_PER_GROUP = 256
...
unsigned int hpc_gpu_adreno_{series}_perfcounter_get_selector(
    hpc_gpu_adreno_{series}_perfcounter_t perfcounter) {{
  {logic}
}}
"""
ADRENO_ENUM_VALUE_TO_SELECTOR_SWITCH_FN = """
unsigned int hpc_gpu_adreno_common_perfcounter_convert_to_{series}(
...
class AdrenoPerSeriesDefinition:
  ...
  def get_all_perfcounters_as_one_enum(self):
    ...
    is assumed to have less than ADRENO_MAX_COUNTER_COUNT_PER_GROUP, which
    ...
    for group_name, group in sorted_groups:
      if len(group) >= ADRENO_MAX_COUNTER_COUNT_PER_GROUP:
      ...
      for case in group.values():
        ...
        value = case.selector + (ADRENO_MAX_COUNTER_COUNT_PER_GROUP *
        ...
        cases.append(
            ADRENO_C_ENUM_CASE_VALUE.format(series=self.series.upper(),
                                            ...
                                            symbol=case.symbol,
                                            value=value))
    return ADRENO_C_ENUM.format(series=self.series, cases="\n  ".join(cases))

  def get_perfcounter_enum_value_to_selector_conversion(self) -> str:
    """Returns a C function converting perfcounter enum values to selectors."""
    logic = "return perfcounter & ({}u - 1u);".format(
        ADRENO_MAX_COUNTER_COUNT_PER_GROUP)
    return ADRENO_ENUM_VALUE_TO_SELECTOR_DIRECT_FN.format(series=self.series,
                                                          logic=logic)
...
def main(args):
  ...
  update_generated_file(
      a5xx_c_lib,
      a5xx_definition.get_perfcounter_enum_value_to_selector_conversion())
  ...
  update_generated_file(
      a6xx_c_lib,
      a6xx_definition.get_perfcounter_enum_value_to_selector_conversion())
