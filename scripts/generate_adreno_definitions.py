...
# Code templates for generating counter C enums
ADRENO_C_ENUM = """
/// Counter choices for Adreno {upper_series} series.
typedef enum hpc_gpu_adreno_{series}_counter_e {{
  {cases}
}} hpc_gpu_adreno_{series}_counter_t;
...
# Code templates for generating counter conversion functions
ADRENO_ENUM_VALUE_TO_SELECTOR_DIRECT_FN = """
static inline uint32_t adreno_{series}_counter_get_selector(
    hpc_gpu_adreno_{series}_counter_t counter) {{
  {selector_logic}
}}
"""
ADRENO_ENUM_VALUE_TO_SELECTOR_SWITCH_FN = """
static uint32_t adreno_common_counter_convert_to_{series}(
...
static inline uint32_t adreno_{series}_counter_get_group(
    hpc_gpu_adreno_{series}_counter_t counter) {{
  {group_logic}
}}
"""
...
class AdrenoPerSeriesDefinition:
  ...
  def get_all_counters_as_one_enum(self):
    ...
    known_groups = [
        ...
        if ADRENO_COUNTER_GROUP_IDS.get(item[0]) is not None
    ]
    uknown_groups = [
        item[0]
        for item in self.groups.items()
        if ADRENO_COUNTER_GROUP_IDS.get(item[0]) is None
    ]
    if len(uknown_groups) != 0:
      print(f"  warning: {self.series} UKNOWN groups: {uknown_groups}")

    cases = []
    sorted_groups = sorted(known_groups,
                           key=lambda item: ADRENO_COUNTER_GROUP_IDS[item[0]])
  ...
  def get_counter_conversion_functions(self) -> str:
    """Returns C functions converting enum values to groups/selectors."""
    selector_logic = "return counter & ({}u - 1u);".format(
        2**ADRENO_MAX_NUM_COUNTER_BIT_PER_GROUP)
    group_logic = "return counter >> {}u;".format(
...
class AdrenoAllSeriesDefinition:
  ...
  def get_common_series(self):
    """Gets a "common" series with counters available to all series."""
